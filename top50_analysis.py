...
def load_top50(annee):
    top50 = pd.read_csv(f"data/year_top_50/{annee}.csv", sep=";")
    return top50
...
def best_genres(top50):
    freq = collections.Counter(top50.apply(genres, axis=1).genres.sum())
    return dict(sorted(freq.items(), key=lambda x: x[1], reverse=True))


def avg_duration(top50):
    minutes, seconds = divmod(round(top50.duration.mean()), 60)
...
def Top50_history(start, end):
    data_top50 = []
    for year in range(start, end):
        try:
            top50 = load_top50(year)
            data = dict()
            data["year"] = year
            data["duration"] = round(top50.duration.mean())
            data["genres"] = best_genres(top50)
            data_top50.append(data)
        except:
            print(f"No top100 in: {year}")
            pass
    df = pd.DataFrame(data_top50)
    return df
...
def music_evolution(start, end, precision=20, file=None, save_genre=None, save_duration=None):
    if file:
        ...
        data = Top50_history(start, end)
        data = Top50_genres(data)
        data = Top50_history_by_year(data)
    plot_duration_evolution(data, precision, save_duration)
    plot_genres_evolution(data, precision, save_genre)


if __name__ == "__main__":
    music_evolution(1961, 2022)
